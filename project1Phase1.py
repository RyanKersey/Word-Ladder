...
def makeNeighborLists(wordList):
    ...
    for x in range(len(wordList)):
        for y in range(len(wordList)):
            S += [wordList[y]]*areNeighbors(wordList[x],wordList[y])
        L.append(S)
        S = []
    return L
...
def getIsolatedEdges(wordList, nbrsList):
    I = []
    for x in range(len(nbrsList)):
        for z in range(len(nbrsList[x])):
            if(len(nbrsList[x]) == 1) and (len(getNeighbors(wordList, nbrsList, nbrsList[x][z])) == 1):
                I += [[wordList[x],getNeighbors(wordList, nbrsList, wordList[x])[0]]]
...
def degreeDistribution(wordList, nbrsList):
    ...
    length = 0
    for x in range(len(nbrsList)):
        if (len(nbrsList[x]) > length):
            length = (len(nbrsList[x]))
    length += 1
